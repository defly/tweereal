...
Activity.utils = {
  ...
  filter : function(w, h, projection) {
    ...
    var points = [
      ...
      new google.maps.Point(w, 0),
      new google.maps.Point(w, h),
    ...
    return [
      [Math.min.apply(null, lat), Math.max.apply(null, lat)],
      [Math.min.apply(null, lng), Math.max.apply(null, lng)]
    ];
  },
  ...
  tweetPhase : function(start, live) {
    // return Math.sqrt((start + live - Date.now()) / live);
        // var t = (start + live - Date.now()) / live;
        // return 1 - ((start + live - Date.now()) / live);
